#!/usr/bin/env python3
"""
Backtest the Scalp strategy (ported from freqtrade-strategies/berlinguyinca/Scalp.py)
=====================================================================================
Pulls real BloFin 1m candles for the last ~24h and runs the ScalpStrategy
(stoch + ADX + EMA mean-reversion scalper) bar-by-bar with $10 starting balance.

Usage:
  python3 backtest_scalp.py                                    # default DOGE
  TRADING_PAIR=ONDO/USDT:USDT python3 backtest_scalp.py
  TRADING_PAIR=SOL/USDT:USDT  python3 backtest_scalp.py
  TRADING_PAIR=SUI/USDT:USDT  python3 backtest_scalp.py
  TRADING_PAIR=XAU/USDT:USDT  python3 backtest_scalp.py
"""
...
from src.scalp_strategy import ScalpStrategy, Signal
...
def run_backtest(df: pd.DataFrame, market: dict):
    strategy   = ScalpStrategy(STRATEGY_CONFIG)
    balance    = INITIAL
    trades     = []
    skipped    = 0
    skip_until = 0
    ...
    while i < len(df) - 1:
        ...
        trades.append({
            ...
            "net": net_pnl, "balance": balance, "bars": bars_held,
        })
...
def display(trades, skipped, df, symbol):
    ...
    print()
    print("=" * W)
    print(f"  SCALP STRATEGY 24H BACKTEST  --  {symbol}")
    print("=" * W)
    print(f"  Source    : LIVE BloFin 1m candles")
    print(f"  Period    : {start_ts.strftime('%a %d %b %Y  %H:%M')} UTC  ->  "
          f"{end_ts.strftime('%a %d %b %Y  %H:%M')} UTC")
    print(f"  Strategy  : Scalp (Stoch+ADX+EMA mean-reversion, ported from freqtrade)")
    print(f"  Risk      : {RISK_PCT*100:.0f}%/trade  |  R:R {RR:.1f}:1  |  Commission 0.12% RT")
    print("-" * W)
    ...
    if trades:
        print()
        print(f"  {'#':<4} {'Time UTC':<13} {'Dir':<6} {'Entry':>11} {'Exit':>11} "
              f"{'Net P&L':>10} {'Result':<11} {'Balance':>11} {'Held':>5}")
        print("  " + "-" * (W - 2))
        for t in trades:
            sign = "+" if t["net"] >= 0 else ""
            print(f"  {t['n']:<4} {t['ts'].strftime('%d %H:%M'):<13} {t['dir']:<6} "
                  f"{t['entry']:>11.6f} {t['exit']:>11.6f} "
                  f"{sign}${t['net']:>+8.4f} {t['outcome']:<11} ${t['balance']:>9.4f} "
                  f"{t['bars']:>4}m")
...
def main():
    ...
    print(f"  Running Scalp strategy on the last 24h ({min(1440, len(df))} bars) ...")
